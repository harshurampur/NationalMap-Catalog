...
// (and an option array of dimension ids whose values should be suppressed in the output),
// check the field names and value names match, and
// output the field names and the hierarchy of allowed values in yaml format.
...
function getSdmxMetaData(ids, sdmxJsonDataFlowUrl, sdmxDataStructureUrl, ignoreJsonDimensionIds) {
    if (!ignoreJsonDimensionIds) {
        ignoreJsonDimensionIds = [];
    }
                ...
                function(dataflow, xml) {
                    ...
                    parser.parseString(xml, function (err, dataStructure) {
                        if (err) {
                            ...
                            console.log(yaml.safeDump(getProcessedMetadata(id, dataflow, dataStructure, ignoreJsonDimensionIds)));
                        }
                    });
                }
            ).catch(function(e) {
...
function getProcessedMetadata(id, dataflow, dataStructure, ignoreJsonDimensionIds) {
